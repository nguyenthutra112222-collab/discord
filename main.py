...
async def roll(ctx, bet: str = None):
    ...
    if player["cash"] < bet_amount:
        return await ctx.send(
            f"❌ Bạn chỉ còn {player['cash']:,} Cash"
        )
    ...
    if rng <= jackpot_limit:
        ...
        if leveled_up:
            await ctx.send(
                f"🎉 {ctx.author.mention} lên Level {player['level']}!"
            )
